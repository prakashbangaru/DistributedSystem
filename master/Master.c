...
#include <netdb.h>
#include "Master.h"
#include "userMonteCarloQuery.c"
#include "clientMonteCarloMapping.c"
#include "queryMessageType.c"
       ...
       int clientSocket = threadAttr->sock;
        ...
        if( (readSize = recv(clientSocket,&messageType,sizeof(messageType) , 0)) > 0 ){
              ...
              if(messageType == MONTECARLO_MESSAGE_TYPE){
                  clientMonteCarloHandler(clientSocket);   
              }
        }
}
...
void clientMonteCarloHandler(int clientSocket){
     ...
     memset(&userMonteCarloQuery,'\0',sizeof(UserMonteCarloQueryTable));

        /*Receive User file Query from Client*/
        if( (readSize = recv(clientSocket , &userMonteCarloQuery , sizeof(UserMonteCarloQueryTable) , 0)) > 0 ){
                  close(clientSocket);
                  printf("Received MonteCarlo Solution from  Client, CircularPoints %d\n",userMonteCarloQuery.totalCircularCount);
                 /*Iyer*/
                 userMonteCarloQuery_set_all(userMonteCarloQuery.userIpAddress, userMonteCarloQuery.userPortNumber, userMonteCarloQuery.queryId, userMonteCarloQuery.totalDistributionCount, userMonteCarloQuery.numberOfClients);
                 clientMonteCarloMapping_set(userMonteCarloQuery.userIpAddress, userMonteCarloQuery.userPortNumber, userMonteCarloQuery.queryId, userMonteCarloQuery.totalCircularCount);
                 queryMessageType_set_all(userMonteCarloQuery.userIpAddress, userMonteCarloQuery.userPortNumber, userMonteCarloQuery.queryId, MONTECARLO_MESSAGE_TYPE);
                 /*send MonteCarlo Solution to the USer*/
                 sendUserMonteCarloSolution(&userMonteCarloQuery);
        }
}
...
void userHandler(QueryDetailsType *queryDetails,int sock,char *ipAddress){
	...
	if( (read_size = recv(sock , queryDetails , sizeof(QueryDetailsType) , 0)) > 0 ){
           ...
           if(queryDetails->queryType == FILE_SORT_MESSAGE_TYPE){
              ...
              userMontecarloHandler(queryDetails);
           }
           else if(queryDetails->queryType == QUERY_MESSAGE_TYPE){
                   userQueryHandler(queryDetails,sock);
           }
           else{
              printf("Invalid Message Type from User\n");
           }
...
void userQueryHandler(QueryDetailsType *queryDetails,int sock){
     ...
     int32_t numberOfSubsolution=0;
     int32_t subsoln_arr[10];
     double piValue = 0.0;
     int index=0;
     int n=0;
     memset(subsoln_arr,'\0',sizeof(int32_t)*10);

     queryMessageType_get_all(queryDetails->ipAddress,queryDetails->portNumber,queryDetails->queryId, &messageType);
    
     if(messageType == MONTECARLO_MESSAGE_TYPE){
           int32_t totalDistributionPoint=0;
           int32_t totalCircularPoint=0;

           clientMonteCarloMapping_get(queryDetails->ipAddress,queryDetails->portNumber,queryDetails->queryId,&numberOfSubsolution,subsoln_arr);
           userMonteCarloQuery_get_all(queryDetails->ipAddress,queryDetails->portNumber,queryDetails->queryId,&totalDistributionPoint,&numberOfClients);
           for(index=0; index< numberOfClients;index++){
                     if(subsoln_arr[index]=='\0'){
                        /*BVP Send Failure Message to the User*/
                        n = write(sock,(void*) &piValue, sizeof(double));
          		if (n < 0){
               		  fprintf(stderr, "Error on sending MonteCarlo Solution  to the user  %s,%s\n",queryDetails->ipAddress,strerror(errno));
               			 exit(EXIT_FAILURE);
       			 }
                         
                        printf("Failed to generate Solution for  Query %d\n",queryDetails->queryId);
                     }
                     printf("SubSolution :%d :%d\n",index, subsoln_arr[index]);
                     totalCircularPoint += subsoln_arr[index];                  
           }
            
          piValue =(double) (totalCircularPoint*4)/(double) totalDistributionPoint; 
         /*Send the MonteCarlo Query*/
          n = write(sock,(void*) &piValue, sizeof(double));
          if (n < 0){
                fprintf(stderr, "Error on sending MonteCarlo Solution  to the user  %s,%s\n",queryDetails->ipAddress,strerror(errno));
                exit(EXIT_FAILURE);
         }
         printf("piValue sent To User %lf\n",piValue);
...
void userMontecarloHandler(QueryDetailsType *queryDetails){
     ...
     int32_t masterDistributionPoint =0;
     int32_t clientDistributionPoint=0;
     int32_t totalDistributionPoint=0;
     int32_t index=0;
     int32_t clientSocket=0;
     int32_t messageType = MONTECARLO_MESSAGE_TYPE;
     int n=0;

     UserMonteCarloQueryTable userMonteCarloQuery; 
        
     totalDistributionPoint = atoi(queryDetails->queryDetail);

      pthread_mutex_lock(&CLIENT_IDENTITY_MUTEX);
      numberOfClients = clientIdentityIndex+1; /*Including the Master*/
      pthread_mutex_unlock(&CLIENT_IDENTITY_MUTEX);
  
      clientDistributionPoint = totalDistributionPoint / numberOfClients;
      masterDistributionPoint = clientDistributionPoint +(totalDistributionPoint % numberOfClients); 

      printf("Distribution Count from USER is : %s\n",queryDetails->queryDetail);

      /*Upate the  userMonteCarloQuery to be Sent to the Client*/
      getUserMonteCarloQuery(&userMonteCarloQuery,queryDetails,numberOfClients,clientDistributionPoint);
     
      for(index =1 ;index < numberOfClients ; index++){
            /*Connect to the Client*/
            clientSocket = connectToClient(clientDetails[index-1].ipAddress,clientDetails[index-1].portNumber);
            /*Send the Message Type*/
            n =  write(clientSocket,(void*) &messageType, sizeof(messageType));
            if (n < 0){
                fprintf(stderr, "Error on sending message Type to the Client  %s,%s\n",clientDetails[index-1].ipAddress,strerror(errno));
                exit(EXIT_FAILURE);
            }
            /*Send the MonteCarlo Query*/
            n = write(clientSocket,(void*) &userMonteCarloQuery, sizeof(UserMonteCarloQueryTable));
            if (n < 0){
      ...
      /*Upate the  userMonteCarloQuery to be Handled By the Master*/
      getUserMonteCarloQuery(&userMonteCarloQuery,queryDetails,numberOfClients,masterDistributionPoint);
   
      /*BVP  Do MonteCarlo Calculation*/
...
void processMonteCarlo(int32_t num_points, int *num_circle)
{
	...
	double x_val= 0.0;
        double y_val=0.0;
        double point_dist=0.0;/*Distance Between two Points*/
	*num_circle=0;
	for (i=0; i<num_points; i++){
		...
		// checking if point lies within the circle
		if (point_dist<=1)
			*num_circle += 1;
	}
	printf("At Master Total points = %d \t num_circle = %d\n", num_points, *num_circle);
}
...
void sendUserMonteCarloSolution(UserMonteCarloQueryTable *userMonteCarloQuery){
     ...
     int32_t messageType= MONTECARLO_MESSAGE_TYPE;
     int n=0;
     userSocket = connectUser(userMonteCarloQuery->userIpAddress,userMonteCarloQuery->userPortNumber);

      /*Send the Message Type*/
      n =  write(userSocket,(void*) &messageType, sizeof(messageType));
      if (n < 0){
           fprintf(stderr, "Error on sending message Type to the User  %s,%s\n",userMonteCarloQuery->userIpAddress,strerror(errno));
           exit(EXIT_FAILURE);
       }
 
      /*Send userMonteCarloQuery Solution to the USer*/
      n = write(userSocket,(void*)userMonteCarloQuery, sizeof(UserMonteCarloQueryTable));
      if (n < 0){
                fprintf(stderr, "Error on sending userMonteCarloQuery solution to User %s,%s\n",userMonteCarloQuery->userIpAddress,strerror(errno));
                exit(EXIT_FAILURE);
       }
...
void getUserMonteCarloQuery(UserMonteCarloQueryTable *userMonteCarloQuery,QueryDetailsType *queryDetails,
            ...
            memcpy(userMonteCarloQuery->userIpAddress,queryDetails->ipAddress,INET_ADDRSTRLEN);
            userMonteCarloQuery->userPortNumber = queryDetails->portNumber;
            userMonteCarloQuery->queryId = queryDetails->queryId;
            userMonteCarloQuery->totalDistributionCount = atoi(queryDetails->queryDetail);
            userMonteCarloQuery->clientsDistributionCount = clientDistributionPoint;
            userMonteCarloQuery->totalCircularCount = 0;
            userMonteCarloQuery->numberOfClients = numberOfClients;
}
